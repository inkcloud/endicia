...
export type EndiciaMailClassType = 'PriorityExpress' | 'First' | 'LibraryMail' | 'MediaMail'
	| 'ParcelSelect' | 'RetailGround' | 'Priority' | 'PriorityMailExpressInternational'
	| 'FirstClassMailInternational' | 'FirstClassPackageInternationalService' | 'PriorityMailInternational'
...
type DefaultEndiciaOptions = Pick<EndiciaOptions, 'mode' | 'requesterId'>

const defaultOptions: DefaultEndiciaOptions  = {
  mode: 'test',
  requesterId: 'lxxx',
}

export class Endicia {
  ...
  requesterId: string;
	...
	async rateSingle(data: ShipRateOption) {
		...
		return new Promise((resolve, reject) => {
			const r = parseString(response, {explicitArray: false}, (err, data) => {
	...
	async getPostageLabel(data: PostageLabelOption) {
		...
		.ele('PartnerTransactionID', data.orderRefernence || shortid.generate()).up()
